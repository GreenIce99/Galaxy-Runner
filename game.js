// ================================
// Galaxy Runner: Universe Edition 3D XL
// ================================
...
// ----- Arrays -----
let enemies=[], enemyBullets=[], powerups=[], particles=[], boss=null;

// ----- Timers -----
let frame=0, countdown=180, waveTimer=0;
...
function spawnParticles(x,y,color,count=20){
  for(let i=0;i<count;i++) particles.push({x,y,vx:rand(-3,3),vy:rand(-3,3),life:rand(20,50),color});
}
...
function updateAll(){
  ...
  if(state===STATE.PLAYING){
    // spawn waves
    waveTimer--;
    if(waveTimer<=0){ spawnEnemy(Math.random()<0.5?'small':'shooter'); waveTimer=80; }
...
// ----- Start game -----
updateAll();
console.log('Galaxy Runner 3D XL loaded — Press Space to shoot!');
